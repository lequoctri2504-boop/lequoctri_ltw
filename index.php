...
<?php $result_products->data_seek(0); $count=0; while($product=$result_products->fetch_assoc()): if($count>=4)break; $count++; $discount=0; if($product['gia']>0){$discount=round((($product['gia']-$product['gia_khuyen_mai'])/$product['gia'])*100);} ?>
...
<div class="container">
<div class="section-header"><h2><i class="fas fa-fire"></i> THƯƠNG HIỆU NỔI BẬT</h2></div>
...
<script>
function addToCart(productId){fetch('<?php echo SITE_URL;?>/api/cart-add.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product_id:productId,quantity:1})}).then(response=>response.json()).then(data=>{if(data.success){showToast('Đã thêm vào giỏ hàng!','success');updateCartCount();}else{showToast(data.message||'Có lỗi xảy ra!','error');}}).catch(error=>{console.error('Error:',error);showToast('Có lỗi xảy ra!','error');});}
</script>
